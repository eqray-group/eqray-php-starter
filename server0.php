...
function checkFilesChange(array $watchDirs, array &$lastMtimes): bool
{
    ...
    // 按操作系统使用对应路径分隔符，避免 Linux 下 is_dir / stripos 失效
    $excludeDirs = $isWin
        ? ['\\.git\\', '\\.idea\\', '\\vendor\\', '\\runtime\\', '\\storage\\', '\\node_modules\\']
        : ['/.git/', '/.idea/', '/vendor/', '/runtime/', '/storage/', '/node_modules/'];
    ...
    foreach ($watchDirs as $dir) {
        ...
        foreach ($it as $file) {
            ...
            $path = $isWin ? str_replace('/', '\\', $file->getRealPath()) : $file->getRealPath();
            if (!$path) continue;

            $filename = $file->getFilename();
            $filepath = $isWin ? str_replace('/', '\\', $file->getPath()) : $file->getPath();
...
function startWorkerProcess() {
    ...
    // 用 escapeshellarg 处理路径，跨平台兼容 Linux / Windows
    $cmd = PHP_BINARY . ' ' . escapeshellarg($script) . ' --worker';
    $descriptorspec = [STDIN, STDOUT, STDOUT];
    // bypass_shell 是 Windows 专有选项，Linux 忽略；不传该选项以保持兼容
    $options = stripos(PHP_OS, 'WIN') !== false ? ['bypass_shell' => true] : [];
    $resource = proc_open($cmd, $descriptorspec, $pipes, null, null, $options);
...
// 如果是工作进程模式，启动 Worker
if (isWorkerProcess()) {
    // Workerman 在 Linux 上 parseCommand() 要求 argv[1] 是合法命令。
    // '--worker' 是自定义标识，Workerman 不识别会打 Usage 后 exit。
    // 在确认 isWorkerProcess() 之后把它改回 'start'，让 Workerman 正常启动。
    global $argv;
    $argv[1] = 'start';
