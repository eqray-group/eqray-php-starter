...
final class SchemaWarmup
{
    ...
    private static function scanModels(string $path, string $namespace): array
    {
        ...
        foreach ($iterator as $file) {
            ...
            if ($ref->isAbstract()
                || ! $ref->isSubclassOf(BaseLaORMModel::class)
     ...
     * 预热单个模型.
     */
    private static function warmupModel(string $modelClass): void
    {
        // 支持动态别名 BaseModel
        if (! is_subclass_of($modelClass, Model::class)
            && ! is_subclass_of($modelClass, BaseLaORMModel::class)
        ) {
            return;
        }
     ...
     *
     * @return array<mixed> */
    private static function loadIndexes(Connection $conn, string $table): array
    {
        $fullTableName = $conn->getTablePrefix() . $table;
        // 先查询所有表，判断目标表是否存在
        try {
     ...
     *
     * @return array<mixed> */
    private static function resolveAuditFields(string $modelClass): array
    {
        $fields = [];
